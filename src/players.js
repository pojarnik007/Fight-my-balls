...
// обновление и отрисовка всех сюрикенов Ниндзи
export function updateShurikens(p, ninja, weapon, opponent, opponentWeapon, opponentHp) {
    for (let i = weapon.arrows.length - 1; i >= 0; i--) {
        ...
        let bounced = false;
        if (a.x < 0 || a.x > p.width) { a.angle = Math.PI - a.angle; bounced = true; }
        if (a.y < 0 || a.y > p.height) { a.angle = -a.angle; bounced = true; }
        ...
        const weaponRadius = Math.max(opponentWeapon.renderW, opponentWeapon.renderH) / 2;

        if (wdist < weaponRadius + a.hitRadius) {
            opponentWeapon.clashFlash = 255;
            playClash();
            weapon.arrows.splice(i, 1); // сюрикен исчезает при парировании
            if(opponent.name === "Shielder") handleShieldHit(ninja, opponent, weapon, opponentWeapon, opponentHp = 1? 2:1, opponentHp);
            continue;
        }
